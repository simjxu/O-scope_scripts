...
# Acquire samples from device, channel and for however many samples
    ...
    dwf.FDwfAnalogInChannelRangeSet(device, channel, c_double(1))      # Configure channel range
...
def get_reference_values(collected_samples, freqarr, sampling_rate, num_samples, axis):
    if axis == 1:
        print("RMS:", sqrt(np.mean(np.square(collected_samples))))
    elif axis == 2:
...
sampling_rate = c_double(20000)
print("Generating signal...")
rgdSamples = custom_waveform(freqarr, shiftarr5000, amplitude=amp, simulated_sampling_rate=100000, time_length=0.01)
generate_samples(dev0, c_int(0), rgdSamples, time_length=0.01)
print("Acquiring data...")
time.sleep(0.2)  # Wait for signal to settle...
dwf.FDwfAnalogInConfigure(dev0, c_bool(True), c_bool(True))  # Configures devices to be on
...
while True:
    ...
    # Convert to the reference values
    ...
    plt.show()
    print("Getting reference values...")
    reference_values1 = get_reference_values(converted_val, freqarr, sampling_rate=sampling_rate.value,
                                             num_samples=nSamples, axis=3)
