...
# Acquire samples from device, channel and for however many samples
    ...
    dwf.FDwfAnalogInChannelRangeSet(device, channel, c_double(0.03))      # Configure channel range
    ...
    return rgpy
# ------------ END acquire_samples() -------------------------------------------------------
def get_reference_values(collected_samples, sampling_rate, num_samples, axis):
    # if axis == 1:
    #     print("RMS:", sqrt(np.mean(np.square(collected_samples)))/0.059749)
    # elif axis == 2:
    #     print("RMS:", sqrt(np.mean(np.square(collected_samples)))/0.042193)
    # else:
    #     print("RMS:", sqrt(np.mean(np.square(collected_samples)))/0.059289)
    # Calculate spectrum
    ...
    # yf = 4.545 / num_samples * np.abs(yf)
    ...
    #     start = int((freqarr[i] - 10) / bin_size)
    #     stop = int((freqarr[i] + 10) / bin_size)
...
# RUN TEST ================================================
# nSamples = 6614
# sampling_rate = c_double(6666.67)
...
# Acquire samples, Device 0 Channel 0
dev0chan0_values = acquire_samples(dev0, c_int(0), sampling_rate, nSamples)
# with open("C:\\Users\\abhinavk\Desktop\\New folder\HALT(12-18-17).txt", 'a') as myfile:
#     myfile.write(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + " ")
#     myfile.write(str(dev0chan0_values) + "\n")

dev0chan0_values = dev0chan0_values - np.mean(dev0chan0_values)
converted_val = np.divide(dev0chan0_values, 0.059289)
...
print("RMS:", np.sqrt(np.mean(np.square(converted_val))))

time.sleep(1)
# GET REFERENCE VAL ----------------------------------------------
print("Getting reference values...")
reference_values1 = get_reference_values(converted_val, sampling_rate=sampling_rate.value,
                                         num_samples=nSamples, axis=1)
